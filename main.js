...
function finalizarQuiz() {
    ...
    // Calcular puntuación y actualizar ranking
    const puntuacion = correctCount * 100;
    const ranking = actualizarRanking(nombreJugador, puntuacion);
...
function actualizarRanking(nombre, puntuacion) {
    ...
    const jugadorExistente = ranking.findIndex(item => item.nombre === nombre);
    
    if (jugadorExistente !== -1) {
        // Si estamos en "Intentar de nuevo", actualizar la puntuación solo si es mejor
        if (puntuacion > ranking[jugadorExistente].puntuacion) {
            ranking[jugadorExistente].puntuacion = puntuacion;
        }
    } else {
        // Agregar nuevo jugador al ranking
